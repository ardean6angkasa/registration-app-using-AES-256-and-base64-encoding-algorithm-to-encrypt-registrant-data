    ...
    <button id="sidebarToggleTop" class="btn btn-link d-md-none rounded-circle mr-3">
                    ...
                    <form action="<?= base_url('/activation'); ?>" method="post"
                        ...
                        <div class="input-group">
                            <input type="text" class="form-control bg-light border-0 small" placeholder="(Search keyword: email)"
                                aria-label="Search" aria-describedby="basic-addon2" name="keyword" value="<?= session()->get('keyword4')?>" autocomplete="off" autofocus>
        ...
        <li class="nav-item dropdown no-arrow d-sm-none">
                            ...
                            <div class="dropdown-menu dropdown-menu-right p-3 shadow animated--grow-in"
                                ...
                                <form class="form-inline mr-auto w-100 navbar-search" action="/activation" method="post">
                                <?= csrf_field();?>
                                    <div class="input-group">
                                        <input type="text" class="form-control bg-light border-0 small"
                                            placeholder="(Search keyword: email)" aria-label="Search"
                                            aria-describedby="basic-addon2" name="keyword" value="<?= session()->get('keyword4')?>" autocomplete="off">
                                        <div class="input-group-append">
                                        <button class="btn btn-outline-dark" type="submit" name="search">
                                                <i class="fas fa-search fa-sm"></i>
                                            </button>
...
<h1 class="h3 mb-2 text-gray-800">Activate & deactivate accounts</h1>
<div class="card shadow mb-4">
    <div class="card-header py-3">
    <div class="flashdata" data-flashdata="<?= session()->getFlashdata('msg') ?>"></div>
        <h6 class="m-0 font-weight-bold text-primary">You can activate and deactivate accounts through this menu</h6>           
    </div>
    <div class="card-body">
    <div align="center">
  <div class="col-sm-5">
<?php if(empty($users)):?>
  ...
  <div class="col">
    <div class="card h-100">
      <img src="/img/<?= $row['photo'];?>" class="card-img-top" style="max-height: 350px; overflow:hidden;">
      <div class="card-body">
      <h5 class="card-title" style="color:black" align='center'><?= $row['email'];?><br>
      Role: <?= 
        $role=$row['status']
        ;?></h5>
        ...
        <?php if($role=='USER'):?>
        Full name: <?= $encrypter->decrypt(base64_decode($row['nama']));?><br>
        <?php if(empty($row['tempat_tanggal_lahir'])):?>
          Birth place & date:
          <br>
                  <?php else:?>
        Birth place & date: <?= $encrypter->decrypt(base64_decode($row['tempat_tanggal_lahir']));?><br>
        <?php endif;?>
        Phone number: <?= $encrypter->decrypt(base64_decode($row['phone']));?>
        <?php else:?>
          <?php endif;?>
        Status: <?= $button= $row['aktif'];?>
    
        <?php if($button=='activated'):?>
          <div align='center'>
          <a href="#" class="btn btn-success btn-md btn-activate rounded-pill" data-id="<?= $row['user_id'];?>"
                        data-mail='<?= $row["email"];?>'
                        data-role='<?= $row["status"];?>'><i class="fas fa-toggle-on"></i>
                      </a> 
        </div>
                      <?php else:?>
                        <div align='center'>
                        <a href="#" class="btn btn-danger btn-md btn-deactivate rounded-pill" data-id="<?= $row['user_id'];?>" data-mail='<?= $row["email"];?>'
                        data-role='<?= $row["status"];?>'><i class="fas fa-toggle-off"></i>
                      </a>
                      </div>
                      <?php endif;?>
        </p>
        ...
        <p class="card-text" style="color:black">
        <small class="text-muted">Joined since, <?= date('F d, Y', $row['created']); ?></small>
...
<div class="modal-content">
    ...
    <form action="<?= base_url('/activation/deactivate'); ?>" method="post">
  
  <div class="modal fade" id="deactivateModal" tabindex="-1" aria-labelledby="exampleModalLabel3" aria-hidden="true" data-backdrop="static" data-keyboard="false">
      <div class="modal-dialog modal-dialog-centered">
          ...
          <div class="modal-header">
              <h5 class="modal-title" id="exampleModalLabel3"  style="color:black">Account deactivation</h5>
              <button type="button" class="btn-close" data-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body">
             <p style="color:black">Are you sure, you want to deactivate this account?</p>
          
            <input type="text" style="color:black" class="form-control email" readonly name='email'>
    ...
    <div class="modal fade" id="activateModal" tabindex="-1" aria-labelledby="exampleModalLabel3" aria-hidden="true" data-backdrop="static" data-keyboard="false">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
            ...
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel3"  style="color:black">Account activation</h5>
                <button type="button" class="btn-close" data-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
               <p style="color:black">Are you sure, you want to activate this account?</p>
            
              <input type="text" style="color:black" class="form-control email" readonly>
  ...
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <div class="modal-content">
      ...
      <div class="modal-body">
      <form action="<?= base_url('/sendmsg/send6'); ?>" method="POST" enctype="multipart/form-data">
    <?= csrf_field();?>
...
<label class="custom-file-label text-preview4" for="file4" style="color:black">Send a certificate (only for a registered account)</label>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      ...
      <form action="<?= base_url('/activation/registration'); ?>" method="POST">
    <?= csrf_field();?>
    <div class="form-floating mb-3">
...
<div class="modal fade" id="commitDelete" aria-hidden="true" aria-labelledby="exampleDelete" tabindex="-1" data-backdrop="static" data-keyboard="false">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      ...
      <div class="modal-body">
                                    <form action="<?= base_url('/activation/delete2'); ?>" method="post">
    <?= csrf_field();?>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      ...
      <div class="modal-body">
      <form action="<?= base_url('/profile/editprofile6'); ?>" enctype="multipart/form-data" method="POST">
    <?= csrf_field();?>
    <input type="hidden" name="user_id" value="<?= session()->get('user_id')?>"> 
    <div class="form-group" align="center">
...
<div class="modal fade" id="removePict" aria-hidden="true" aria-labelledby="removePicture" tabindex="-1" data-backdrop="static" data-keyboard="false">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      ...
      <form action="<?= base_url('/removephoto/remove6'); ?>" method="POST">
...
<div class="modal fade" id="exampleModalToggle3" aria-hidden="true" aria-labelledby="exampleModalToggleLabel3" tabindex="-1" data-backdrop="static" data-keyboard="false">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      ...
      <div class="modal-body">
      <form action="<?= base_url('/editpass/password6'); ?>" method="POST">
    <?= csrf_field();?>
    <input type="hidden" name="user_id" value="<?= session()->get('user_id')?>">  
    <div class="form-floating mb-3">
              <input type="password" class="form-control <?php if(isset($validation)):?>
...
<script>
    $(document).ready(function(){
        ...
        $('.btn-edit').on('click',function(){
            ...
            $('.user_id').val(id);
            $('.certificate').val(certif);
            $('.score').val(grade);
            $('.pass_category').val(category).trigger('change');
            $('.catatan').val(msg);
            $('.status').val(role);
            // Call Modal Edit
